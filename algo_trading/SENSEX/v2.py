...
# Assuming these are in your local scalper.py
...
PAPER_DATA_FILE = "paper_account_sensex.json"
...
class PaperEngine(ExecutionEngine):
    ...
    def _fetch_ltp_internal(self, api_symbol: str) -> Optional[float]:
        """Fetches LTP using the API Trading Symbol (e.g. SENSEX23JUL2677850CE)"""
        try:
            opt_data = self.groww.get_historical_candles(
                exchange=self.groww.EXCHANGE_BSE, segment=self.groww.SEGMENT_FNO, groww_symbol=api_symbol,
                start_time=(datetime.now() - timedelta(minutes=2)).strftime("%Y-%m-%d %H:%M:%S"),
                end_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                candle_interval=self.groww.CANDLE_INTERVAL_MIN_1
            )
            # SAFETY CHECK: Ensure candles exist
            if opt_data and opt_data.get('candles') and len(opt_data['candles']) > 0:
                return opt_data['candles'][-1][4]
            else:
                logging.warning(f"Paper LTP Fetch Empty for {api_symbol}: {opt_data}")
                return None
        except Exception as e:
            logging.error(f"Paper LTP Fetch Failed {api_symbol}: {e}"); return None

    def place_entry_order(self, api_symbol: str, qty: int, side: str) -> Tuple[bool, float, str]:
        ltp = self._fetch_ltp_internal(api_symbol)
        if ltp is None: 
            logging.error(f"📝 [PAPER] Cannot place entry, LTP fetch failed for {api_symbol}")
            return False, 0.0, ""
        
        slip = ltp * (PAPER_SLIPPAGE_PCT / 100)
        fill_price = round(ltp + slip if side == "BUY" else ltp - slip, 1)
        ...
        oid = self._gen_id()
        logging.info(f"📝 [PAPER] ENTRY FILLED {side} {qty} {api_symbol} @ ₹{fill_price} (LTP: ₹{ltp})")
        send_telegram_alert(f"🟢 *PAPER ENTRY FILLED (SENSEX)*\n`{api_symbol}`\nQty: {qty} | Side: {side}\nPrice: ₹{fill_price}")
        return True, fill_price, oid

    def place_sl_order(self, api_symbol: str, qty: int, trigger_price: float, side: str) -> Tuple[bool, str]:
        trigger_price = round(trigger_price * 20) / 20
        sl_id = self._gen_id()
        logging.info(f"📝 [PAPER] SL PLACED {api_symbol} Trigger: ₹{trigger_price} | ID: {sl_id}")
    ...
    def check_position_closed(self, api_symbol: str) -> bool:
        if not self.account.current_position: return True
        pos = self.account.current_position
        if pos['symbol'] != api_symbol: return True
        ltp = self._fetch_ltp_internal(api_symbol)
    ...
    def get_ltp(self, api_symbol: str) -> Optional[float]: return self._fetch_ltp_internal(api_symbol)
...
class LiveEngine(ExecutionEngine):
    def __init__(self, groww):
        self.groww = groww; self.active_sl_order_id = None; self.active_api_symbol = None; self.active_side = None
        logging.info("🔴 LIVE ENGINE INITIALIZED (BSE SENSEX) - REAL MONEY")

    def _api_to_trading_symbol(self, api_symbol: str) -> str:
        """
        Converts API Symbol (SENSEX23JUL2677850CE) to Groww Order Trading Symbol.
        Groww Order API usually expects: SENSEX23JUL2677850CE (Same as API symbol for BSE FNO usually)
        If your growwapi version expects different, adjust here.
        """
        # The API symbol generated below (SENSEX{EXPIRY}{STRIKE}{TYPE}) IS the trading symbol for BSE.
        return api_symbol

    def place_entry_order(self, api_symbol: str, qty: int, side: str) -> Tuple[bool, float, str]:
        try:
            ts = self._api_to_trading_symbol(api_symbol)
    ...
    def place_sl_order(self, api_symbol: str, qty: int, trigger_price: float, side: str) -> Tuple[bool, str]:
        try:
            ...
            if resp.get("success") and resp.get("order_id"):
                oid = resp["order_id"]; self.active_sl_order_id = oid; self.active_api_symbol = api_symbol; self.active_side = side
    ...
    def get_ltp(self, api_symbol: str) -> Optional[float]: return None

    def square_off_all(self):
        if self.active_api_symbol:
            logging.info(f"🔴 [LIVE] EOD SQUARE OFF: {self.active_api_symbol}"); send_telegram_alert(f"🏁 *LIVE EOD SQUARE OFF* `{self.active_api_symbol}`")
            if self.active_sl_order_id: self.cancel_order(self.active_sl_order_id); time.sleep(0.2)
            try:
                ts = self._api_to_trading_symbol(self.active_api_symbol)
                positions = self.groww.get_positions()
                if positions and positions.get("data"):
                    for pos in positions["data"]:
            ...
            self.active_sl_order_id = None; self.active_api_symbol = None; self.active_side = None
...
def get_futures_expiry(groww) -> str:
    """
    Returns the Monthly Futures Expiry string in 'DDMMMYY' format (e.g., '30Jul26').
    Used for Futures Symbol construction (BSE-SENSEX-30Jul26-FUT).
    """
    today = date.today()
    current_month_last_thu = get_last_thursday(today.year, today.month)
    
    if current_month_last_thu >= today:
        expiry_date = current_month_last_thu
    else:
    ...
    fmt = expiry_date.strftime("%d%b%y") # e.g., 30Jul26
    logging.info(f"[Futures Expiry Calculated] {fmt} (Date: {expiry_date})")
    return fmt

def get_nearest_options_expiry(groww) -> date:
    """
    Fetches expiries from API and returns the NEAREST Weekly/Monthly expiry DATE OBJECT for OPTIONS.
    """
    now = datetime.now()
    all_expiries = []
    ...
    for y, m in months_to_check:
        try:
            ...
            logging.warning(f"Options Expiry fetch failed for {m}/{y}: {e}")

    if not all_expiries:
        logging.error("NO VALID OPTION EXPIRIES FOUND. Using Futures Expiry as fallback.")
        # Fallback: calculate monthly expiry date
        return get_last_thursday(today.year, today.month) if (today := date.today()) else date.today()

    return min(all_expiries)

def get_current_futures_symbol(groww) -> str:
    """Constructs the ACTIVE Monthly Futures Symbol for Sensex (Format works for Historical API)"""
    expiry = get_futures_expiry(groww) 
    symbol = f"{INDEX_PREFIX}-{expiry}-FUT"
    logging.info(f"Active Futures Symbol Resolved: {symbol}")
    return symbol

def fetch_dataframe(groww, api_symbol: str, interval, days_back):
    """
    Fetches candles using the API Symbol (Trading Symbol for Options, BSE-... for Futures).
    """
    ...
    try:
        data = groww.get_historical_candles(
            ...
            groww_symbol=api_symbol, # Pass the correct symbol format directly
        ...
        if not data or "candles" not in data or not data.get("candles"):
            logging.warning(f"API Empty/Invalid Response for {api_symbol}: {data}")
            return pd.DataFrame()
        ...
        if not clean_candles:
            logging.warning(f"No valid candle rows for {api_symbol}")
            return pd.DataFrame()
        ...
        logging.error(f"Exception in fetch_dataframe for {api_symbol}: {e}")
        return pd.DataFrame()
...
def manage_dynamic_trade(engine: ExecutionEngine, groww, api_symbol: str, entry_price: float, current_vix: float, side: str):
    ...
    success, sl_order_id = engine.place_sl_order(api_symbol, QUANTITY, current_sl, side)
    if not success:
    ...
    if isinstance(engine, PaperEngine):
        ...
        engine.account.open_position(api_symbol, side, entry_price, QUANTITY, current_sl, sl_order_id, est_margin)
    ...
    while True:
        ...
        try:
            # Fetch LTP using API Symbol
            opt_data = groww.get_historical_candles(
                exchange=groww.EXCHANGE_BSE, segment=groww.SEGMENT_FNO, groww_symbol=api_symbol,
            ...
            # SAFETY CHECK
            if not opt_data or not opt_data.get('candles') or len(opt_data['candles']) == 0:
                logging.warning(f"Monitor: Empty candle data for {api_symbol}. Retrying...")
                time.sleep(1)
                continue
            ...
            if isinstance(engine, PaperEngine):
                ...
                if int(time.time()) % 30 == 0:
                    ...
                    if pos: logging.info(f"Tracking {api_symbol} | LTP: ₹{live_premium} | SL: ₹{current_sl} | uPnL: ₹{engine.account.unrealized_pnl:.2f}")

            if engine.check_position_closed(api_symbol):
...
def execute_strategy(groww, engine: ExecutionEngine):
    ...
    try:
        # 1. Get CORRECT Monthly Futures Symbol (Format: BSE-SENSEX-30Jul26-FUT)
        fut_symbol = get_current_futures_symbol(groww) 
        ...
        latest = df_1m.iloc[-2] # Closed Candle
        ...
        if trade_type:
            ...
            # 2. Get NEAREST Options Expiry (Date Object)
            opt_expiry_date = get_nearest_options_expiry(groww)
            
            # 3. Construct API/Trading Symbol for Option: SENSEX{DDMMMYY}{STRIKE}{CE/PE}
            # Example: SENSEX23JUL2677850CE
            # Expiry format for trading symbol: DDMMMYY (Uppercase) -> 23JUL26
            expiry_str_trading = opt_expiry_date.strftime("%d%b%y").upper() 
            option_api_symbol = f"{UNDERLYING}{expiry_str_trading}{atm_strike}{option_suffix}"
            
            # Display Symbol for logs (Human readable)
            display_symbol = f"{INDEX_PREFIX}-{opt_expiry_date.strftime('%d%b%y')}-{atm_strike}-{option_suffix}"
            
            alert = f"🚀 *{trade_type} (SENSEX 1M)*\nMode: **{TRADING_MODE}**\nFut: ₹{close_price}\nVWAP/RSI Aligned | VIX: {current_vix:.2f}\nTarget: `{display_symbol}`\nAPI Symbol: `{option_api_symbol}`"
            logging.info(alert); send_telegram_alert(alert)
            
            # Pass the API Symbol (Trading Symbol) to engine
            success, fill_price, oid = engine.place_entry_order(option_api_symbol, QUANTITY, entry_side)
            
            if TRADING_MODE == "PAPER" and fill_price == 0.0:
                # Fallback fetch using API Symbol
                opt_df = fetch_dataframe(groww, option_api_symbol, groww.CANDLE_INTERVAL_MIN_1, 1)
                if not opt_df.empty: fill_price = opt_df.iloc[-1]['close']

            if success:
                manage_dynamic_trade(engine, groww, option_api_symbol, fill_price, current_vix, entry_side)
                return True
            else:
                logging.error("Entry Order Failed. Skipping trade management.")
                return False
        return False
    except Exception as e: 
...
if __name__ == "__main__":
    # Use your actual login function
    ...
    if groww_client:
        ...
        while True:
            ...
            if current_time >= market_open and current_time < market_close and (eod_printed or weekend_printed):
                eod_printed = False; weekend_printed = False
            ...
            if (0 <= current_day <= 4) and (market_open <= current_time <= market_close):
                ...
                if trade_executed:
                    ...
                    while datetime.now().time() < market_close: time.sleep(30)
            else:
