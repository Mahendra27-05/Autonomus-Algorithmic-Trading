...
# Assuming these are in your local scalper.py, keeping imports as is.
...
class LiveEngine(ExecutionEngine):
    ...
    def _data_to_trading_symbol(self, data_symbol: str) -> str:
        try:
            ...
            if len(parts) == 5:
                ...
                expiry_fmt = dt.strftime("%y%b%d").upper()
    ...
    def get_ltp(self, data_symbol: str) -> Optional[float]: return None
...
def get_last_thursday(year: int, month: int) -> date:
    ...
    # Start from 1st of next month, go back 1 day to get last day of current month
    if month == 12:
...
def get_futures_expiry(groww) -> str:
    ...
    Sensex Futures are MONTHLY (Last Thursday).
    """
    today = date.today()
    ...
    # Format: DDMMMYY (e.g., 30Jul26)
    fmt = expiry_date.strftime("%d%b%y")
    logging.info(f"[Futures Expiry Calculated] {fmt} (Date: {expiry_date})")
    return fmt

def get_nearest_options_expiry(groww) -> str:
    """
    Fetches expiries from API and returns the NEAREST Weekly/Monthly expiry for OPTIONS.
    Format: 'DDMMMYY' (e.g., '23Jul26')
    """
    now = datetime.now()
    all_expiries = []
    ...
    if not all_expiries:
        ...
        return get_futures_expiry(groww)

    nearest = min(all_expiries)
    return nearest.strftime("%d%b%y")

def get_current_futures_symbol(groww) -> str:
    """Constructs the ACTIVE Monthly Futures Symbol for Sensex"""
    expiry = get_futures_expiry(groww) # <--- USES MONTHLY LOGIC
...
def execute_strategy(groww, engine: ExecutionEngine):
    ...
    try:
        # 1. Get CORRECT Monthly Futures Symbol
        ...
        if trade_type:
            ...
            # 2. Get NEAREST Options Expiry (Weekly/Monthly) for Option Buying
            opt_expiry = get_nearest_options_expiry(groww)
            target_symbol = f"{INDEX_PREFIX}-{opt_expiry}-{atm_strike}-{option_suffix}"
...
if __name__ == "__main__":
    # Assuming login_to_groww is imported from scalper
    # from scalper import login_to_groww
    # For standalone run, define a mock or ensure scalper.py exists.
    # groww_client = login_to_groww() 
    
    # MOCK LOGIN FOR STRUCTURE CHECK (Replace with your actual login_to_groww)
    class MockGroww:
        EXCHANGE_BSE = "BSE"; SEGMENT_FNO = "FNO"; CANDLE_INTERVAL_MIN_1 = "1m"
        ORDER_TYPE_MARKET = "MARKET"; ORDER_TYPE_SL_M = "SL-M"; PRODUCT_TYPE_MIS = "MIS"; VALIDITY_DAY = "DAY"
        TRANSACTION_TYPE_BUY = "BUY"; TRANSACTION_TYPE_SELL = "SELL"
        def get_historical_candles(self, **kwargs): return {"candles": []}
        def get_expiries(self, **kwargs): return {"expiries": ["2026-07-23", "2026-07-30"]}
        def get_positions(self): return {"data": []}
        def place_order(self, **kwargs): return {"success": True, "order_id": "MOCK_123"}
        def get_order_history(self, **kwargs): return {"data": []}
        def modify_order(self, **kwargs): return {"success": True}
        def cancel_order(self, **kwargs): return {"success": True}

    # Uncomment below for actual run
    groww_client = login_to_groww() 
    
    # groww_client = MockGroww() # For testing logic without API
