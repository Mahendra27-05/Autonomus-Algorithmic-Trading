...
# FUTURES_EXPIRY and FUTURES_SYMBOL are now DYNAMIC (set in main())
QUANTITY = 65 
...
# Global placeholder for Futures Symbol (Populated in main)
FUTURES_SYMBOL = None 
...
# ==========================================
# 2. VIP DATABASE & TELEGRAM (UNCHANGED)
# ==========================================
...
class PaperAccount:
    ...
    def get_status_message(self) -> str:
        ...
        return (f"📊 *PAPER ACCOUNT STATUS*\n💰 Equity: ₹{eq:,.2f} (Start: ₹{self.starting_capital:,.0f})\n"
                f"📈 Daily PnL: ₹{daily_pnl:,.2f} ({daily_pct:+.2f}%)\n💵 Free Cash: ₹{self.get_free_cash():,.2f} | Margin Used: ₹{self.used_margin:,.2f}\n"
...
class PaperEngine(ExecutionEngine):
    ...
    def check_position_closed(self, trading_symbol: str) -> bool:
        ...
        try:
            ...
            if hit: logging.warning(f"📝 [PAPER] SL TRIGGERED! LTP: {ltp} <= SL: {sl}"); return True
        except Exception as e: logging.error(f"Paper SL Check Error: {e}")
        return False
    ...
    def place_entry_order(self, trading_symbol: str, qty: int, side: str) -> Tuple[bool, float, str]:
        try:
            ...
            if resp.get("success") and resp.get("order_id"):
                ...
                trades = self.groww.get_order_history(order_id=oid); avg_price = 0.0
...
# ==========================================
# 5. HELPER & DATA FUNCTIONS (MODIFIED FOR WEEKLY EXPIRY)
# ==========================================
def login_to_groww():
    try:
...
# --- MODIFIED: Weekly Expiry Logic ---
def get_nearest_options_expiry(groww, underlying="NIFTY"):
    """
    Fetches expiries for Current & Next Month, filters for Thursdays (Weeklies),
    returns nearest upcoming Weekly Expiry formatted as 'DDMonYY' (e.g., '04Jul24').
    """
    now = datetime.now()
    all_expiries = []
    ...
    def fetch_exp(y, m):
        try:
            exp_data = groww.get_expiries(exchange=groww.EXCHANGE_NSE, underlying_symbol=underlying, year=y, month=m)
            return exp_data.get("expiries", [])
        except Exception: return []

    # Fetch Current Month
    all_expiries.extend(fetch_exp(now.year, now.month))
    
    # Fetch Next Month (handle Dec->Jan rollover)
    next_month = now.month + 1; next_year = now.year
    if next_month > 12: next_month = 1; next_year += 1
    all_expiries.extend(fetch_exp(next_year, next_month))

    if not all_expiries:
        logging.error("No expiries fetched from API. Falling back to hardcoded.")
        return "27Jun24" 
    ...
    try:
        ...
        for exp_str in all_expiries:
            try:
                ...
                # NSE Weekly Expire every Thursday (weekday 3)
                if d >= now.date() and d.weekday() == 3:
                    expiry_dates.append(d)
            except ValueError: continue
        
        if not expiry_dates:
            logging.warning("No Thursday expiries found. Taking nearest future date as fallback.")
            future_dates = [datetime.strptime(e, "%Y-%m-%d").date() for e in all_expiries if datetime.strptime(e, "%Y-%m-%d").date() >= now.date()]
            if future_dates: return min(future_dates).strftime("%d%b%y")
            return "27Jun24"

        nearest_thursday = min(expiry_dates)
        formatted = nearest_thursday.strftime("%d%b%y") 
        logging.info(f"📅 Weekly Expiry Selected: {formatted} (Date: {nearest_thursday})")
        return formatted
    except Exception as e:
        logging.error(f"Expiry Parsing Error: {e}"); return "27Jun24"
...
def execute_strategy(groww, engine: ExecutionEngine):
    ...
    try:
        # Uses Global FUTURES_SYMBOL (Set dynamically in main)
        if not FUTURES_SYMBOL:
            logging.error("FUTURES_SYMBOL not initialized.")
            return False
        ...
        if trade_type:
            ...
            # MODIFIED: Calls the new Weekly Expiry Function
            ...
            alert_msg = (f"🚀 *{trade_type} DETECTED (1M FRAME)*\nMode: **{TRADING_MODE}**\n"
                         ...
                         f"Expiry: {dynamic_expiry} | Strike: {atm_strike} {option_suffix}\n"
                         f"Executing BUY for `{target_symbol}`.")
...
# ==========================================
# 7. MAIN LOOP (WITH DYNAMIC FUTURES SYMBOL)
# ==========================================
if __name__ == "__main__":
    ...
    if groww_client:
        # ==========================================================
        # DYNAMIC FUTURES SYMBOL SETUP (Current Month Future)
        # ==========================================================
        try:
            now = datetime.now()
            fut_exp_data = groww_client.get_expiries(exchange=groww_client.EXCHANGE_NSE, underlying_symbol="NIFTY", year=now.year, month=now.month)
            fut_list = fut_exp_data.get("expiries", [])
            fut_dates = [datetime.strptime(d, "%Y-%m-%d").date() for d in fut_list if datetime.strptime(d, "%Y-%m-%d").date() >= now.date()]
            if fut_dates:
                # Current Month Future Expiry (Last Thursday of month)
                current_month_future_expiry = max(fut_dates).strftime("%d%b%y") # e.g., 27Jun24
            else:
                current_month_future_expiry = now.strftime("%d%b%y") # Fallback
            
            FUTURES_SYMBOL = f"NSE-NIFTY-{current_month_future_expiry}-FUT"
            logging.info(f"📈 Futures Symbol Set for Trend: {FUTURES_SYMBOL}")
        except Exception as e:
            logging.error(f"Futures Symbol Setup Failed: {e}. Using Fallback.")
            FUTURES_SYMBOL = "NSE-NIFTY-27Jun24-FUT" # Fallback
        ...
        # Flags to prevent spamming EOD/Weekend messages
        ...
        while True:
            ...
            if (0 <= current_day <= 4) and (market_open <= current_time <= market_close):
                ...
                # After Market Hours / Weekend Logic
